        ...
        */
        for(int z = 0; z < 18; z++)
        {
            for(int y = 0; y < rowCounter; y++)
            {
                ...
                if(records[y].valueReady && records[y].mainValue == operators[z].mainValue && operators[z].variable == records[y].firstVariable && records[y].skipThisDuringSequence == false && !records[y].nop)
                {
                    if(records[y+1].isNotNull && records[y-1].nop){
                        //cout << "CHANGE HERE" << endl;
                        records[y+1].instructionCommand = records[y].instructionCommand;
                    }
                    if((records[y-1].instructionCommand == "and" && records[y-1].skipThisDuringSequence == false) || records[y].instructionCommand == "and")
                    {
                        int commas = 0;
                        int val1 = 0;
                        int val2 = 0;
                        ...
                        operators[z].value = ((val1 > 0) && (val2 > 0));
                    }else if(!records[y-1].nop && (records[y-1].instructionCommand == "add" || records[y-1].instructionCommand == "addi") && records[y-1].skipThisDuringSequence == false && records[y-1].operatorValue != 0)
                    {
                        //cout << "PASS" << endl;
                        operators[z].value += records[y-1].operatorValue;
                    } else if(/*((records[y-1].instructionCommand == "add" || 
                        records[y-1].instructionCommand == "addi") && 
                    records[y-1].skipThisDuringSequence == false) || */
                    (records[y].instructionCommand == "add" ||
                     records[y].instructionCommand == "addi" || 
                     (records[y].instructionCommand == "ori" && y == 0) || 
                     (records[y].instructionCommand == "ori" && y == 1)) && 
                    records[y].operatorValue != 0 )
                    {
                    ...
                    //cout << "THE INSTRUCTION BEING SKIPPED IS " << records[y].instructionCommand << endl;
                }
            }
